    ...
    <header className="fixed top-5 left-1/2 z-50 w-[min(1100px,90%)] -translate-x-1/2">
      <div className="flex items-center justify-between gap-4 rounded-full border border-border bg-white px-5 py-3 shadow-[0_4px_20px_rgba(0,0,0,0.15)] md:px-6">
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          {links.map((l) => (
            <Link
              key={l.to}
              to={l.to}
              className="nav-active-underline text-sm font-medium text-foreground/80 transition-colors hover:text-brand-green"
              activeProps={{ className: "text-brand-green font-semibold active" }}
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/contactos"
            className="hover-btn hidden rounded-xl bg-brand-green px-5 py-2.5 text-sm font-semibold text-white shadow-sm md:inline-flex"
          >
            Solicitar Crédito
          </Link>
          <button
